...
function ForgetPasswordForm() {
  ...
  const onSubmit = async (data: IResetPasswordData) => {
    ...
    const { message, isError } = await authService.forgotPassword(data.email, company_name)
    debugger
    if (!isError) {
  ...
  return (
    <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
      <Spacer>
        ...
        <Button onClick={() => navigate(AUTH_ROUTES.login)} id="loginPagebtn" className="ResetPassBtn">
          Go to Login
        </Button>
      </Spacer>
    </FormProvider>
